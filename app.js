...
const COPY = {
  en: {
    ...
    providedImageNamePriceQuantity: 'Please provide image, name, price, and quantity.',
    validNamePriceQuantity: 'Please enter a valid name, price, and quantity.',
  ...
  'zh-Hans': {
    ...
    providedImageNamePriceQuantity: '请填写图片、名称、价格和数量。',
    validNamePriceQuantity: '请输入有效的名称、价格和数量。',
  ...
  'zh-Hant': {
    ...
    providedImageNamePriceQuantity: '請填寫圖片、名稱、價格和數量。',
    validNamePriceQuantity: '請輸入有效的名稱、價格和數量。',
...
function renderRole() {
  ...
  dom.adminTools.hidden = !state.adminUnlocked;
...
function renderInventoryCards() {
  ...
  dom.staffCardGrid.innerHTML = state.inventory.map(staffCardMarkup).join('');
...
async function handleAddPainting(event) {
  ...
  if (!name || !Number.isFinite(price) || !Number.isFinite(quantity) || quantity < 0 || !imageFile) {
...
async function handleUpdatePainting(itemId) {
  ...
  try {
    const { error } = await supabase
      .from('paintings')
      .update({ name, price, quantity, updated_at: new Date().toISOString() })
      .eq('id', itemId);
