...
- Save serial number, cost, and painting size (height/width in inches).
- Let staff record how many were sold.
- Automatically reduce stock after each sale.
- Keep a live sales log.
- Staff view includes fast search by painting name or serial number.
...
create table if not exists paintings (
  ...
  height_inch numeric not null default 0,
  width_inch numeric not null default 0,
...
If your `paintings` table already exists, run this migration once:

```sql
alter table paintings add column if not exists serial_number text;
alter table paintings add column if not exists cost numeric not null default 0;
alter table paintings add column if not exists height_inch numeric not null default 0;
alter table paintings add column if not exists width_inch numeric not null default 0;

update paintings
set serial_number = coalesce(nullif(serial_number, ''), 'UNKNOWN-' || substr(id::text, 1, 8))
where serial_number is null or serial_number = '';

alter table paintings alter column serial_number set not null;
```
```
